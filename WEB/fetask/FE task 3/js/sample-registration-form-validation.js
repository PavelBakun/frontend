function formValidation() {
    var userAge = document.registration.userAge;
    var passId = document.registration.passId;
    var uname = document.registration.userName;
    var uemail = document.registration.email;


    if (userAge_validation(userAge, 7, 120)) {
...
function userAge_validation(userage, min, max) {
    var uAge = userAge = parseInt(userage.value);
    if (uAge >= min && uAge <= max) {
        return true;
    } else {
        alert("User Age should be between " + min + " to " + max);
        userage.focus();
        return false;
    }
}
